...
| Native app launch smoke | `area:desktop` | exact staged arm64/x64 assets launch natively on `macos-26` and `macos-26-intel` after checksum, signature, notarization, and exact Apple Team ID verification; publication remains blocked until both pass, and recovery promotion repeats the same gates against immutable assets |
...
| Self-hosted docs live | `area:docs` | `https://mutx.dev/docs/releases/v1.5` is reachable after merge |
...
2. Confirm `docs/releases/v1.5.md` is merged, `https://mutx.dev/docs/releases/v1.5` is live, and `https://mutx.dev/releases` reflects the same release.
...
Retain the v1.4 release trust contract: signed annotated tags reachable from `origin/main`, matching
source versions, immutable releases, per-tag concurrency, required-reviewer `release-publishing`
and `production` Environments, exact versioned notes, recoverable draft staging, native arm64/x64
launch gates before publication (and again for recovery), four-artifact streamed checksum
verification including valid SemVer prereleases, exact Apple Team ID checks, full-SHA action pins,
and downgrade-blocking promotion whose same-version path is commit-bound and forward-only. The
desktop support floor remains macOS 12 Monterey on Electron `43.2.0` until a later Electron line
