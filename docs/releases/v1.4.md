...
> This page is a historical release snapshot. For the current route, role, and
> deployment contract, use the API and architecture sections of these docs.
...
- **LangChain:** Replaced stub `stream_events()` with a callback-backed async generator — a background task emits observed LLM, tool, action, and finish callbacks in order, with listener removal and task cancellation when consumers close the stream
...
- Dynamic role management (the current persisted role set is `ADMIN`, `AUDIT_ADMIN`, `DEVELOPER`, and `VIEWER`)
- Additional mounted SSO providers and direct external bearer-token integration beyond the
  Google, GitHub, and Okta account-linking flows
...
- the macOS download lane, which continues to expose only the newest complete stable desktop
  artifact set; v1.4.0 itself did not publish desktop binaries and is not presented as a desktop build
...
- Docs release note: `https://mutx.dev/docs/releases/v1.4`
- GitHub release: `https://github.com/mutx-dev/mutx-dev/releases/tag/v1.4.0`
- Stable dashboard entry: `https://app.mutx.dev/dashboard`

The v1.4.0 GitHub release contains GitHub's source archives, not the five-file macOS desktop
contract. The public download resolver therefore ignores v1.4.0 and retains the newest older release
whose Apple silicon and Intel DMGs, ZIPs, and exact checksum manifest are all verifiable.
